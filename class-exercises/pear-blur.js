...
// 1. three critical lines of code have been commented out, find them, discuss what they do, and comment them back in
// 2. once the demo is running, you should see that it is visually messed up. figure out why and fix it.
...
void main() {
    ...
    uv.y = 1.0 - uv.y;
		...
		vec4 cTop=texture2D(u_sampler, uv - vec2(0,0.99));
		vec4 cBot=texture2D(u_sampler, uv + vec2(0,0.99));
		vec4 cLeft=texture2D(u_sampler, uv - vec2(0.99,0));
		vec4 cRight=texture2D(u_sampler, uv + vec2(0.99,0));
		
		// vec4 cTop=texture2D(u_sampler, uv - vec2(0,0.1));
		// vec4 cBot=texture2D(u_sampler, uv + vec2(0,0.1));
		// vec4 cLeft=texture2D(u_sampler, uv - vec2(0.1,0));
		// vec4 cRight=texture2D(u_sampler, uv + vec2(0.1,0));
		
		color=(color+cTop+cBot+cLeft+cRight)/5.;
...
async function main() {
    ...
    const texture = loadTexture(gl, 'https://compform.net/experimental_shaders/textures/textures/pear.jpg', gl.MIRROR_REPEAT, gl.LINEAR_MIPMAP_LINEAR);
